...
## 6. 已移除的第三方代码 —— comfyUI-llama-TE（tl2012tl）

| 项 | 内容 |
|---|---|
| 原始作者 | tl2012tl |
| 原始仓库 | https://github.com/tl2012tl/comfyUI-llama-TE |
| 曾涉及文件 | `llamate/`（7 个文件）、`web/qwen_multi_turn_chat.js` |
| 许可证 | **上游仓库未提供任何许可证文件**，法律上默认为「保留所有权利」 |
| 当前状态 | **已从本仓库移除**（2026-09-12） |

早期版本曾把这套 Qwen / Gemma4 本地文本编码与多轮对话节点（节点 ID `QwenTE_*` /
`Gemma4TE_*`）合并进本插件。上游没有给出任何授权声明，未经许可继续分发在法律上有风险，
因此本版本将其整体删除：

- 这两个模块在本插件里**没有任何调用点**，删除后插件功能不受影响。
- 需要这些能力时，请自行安装上游的 `comfyUI-llama-TE`，两套节点互不冲突。
- 开发过程中参考过它的本地推理与多轮对话设计，鸣谢保留在 `ACKNOWLEDGEMENTS.md` 第 3 节
  与 `RELEASE.md` 中 —— **参考设计**与**分发代码**是两件事：前者照实感谢，后者必须有授权。
