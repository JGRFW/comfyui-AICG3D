...
const PROMPT_OPTIMIZER_LOCAL_MODELS_ENDPOINT = "/minimax_h3_easy/prompt_optimizer_local_models";
const PROMPT_OPTIMIZER_ENGINE_API = "api";
const PROMPT_OPTIMIZER_ENGINE_LOCAL = "local";
...
let promptOptimizerSettingsOpening = null;
let promptOptimizerSettingsOpenedAt = 0;
let promptOptimizerLocalModelsCache = null;
let promptOptimizerLocalModelsPromise = null;
...
const TEXT = {
    ...
    settingsOpen: ZH_BROWSER ? "\u6253\u5f00\u63d0\u793a\u8bcd\u4f18\u5316\u8bbe\u7f6e" : "Open prompt optimization API settings",
    ...
    optimizerMissingLocal: ZH_BROWSER ? "\u8bf7\u5148\u5728\u63d0\u793a\u8bcd\u4f18\u5316\u8bbe\u7f6e\u4e2d\u9009\u62e9\u672c\u5730\u89c6\u89c9\u6a21\u578b\u3002" : "Select a local vision model in the optimizer settings first.",
    optimizerEngine: ZH_BROWSER ? "\u4f18\u5316\u65b9\u5f0f" : "Optimization engine",
    localModel: ZH_BROWSER ? "\u672c\u5730\u6a21\u578b" : "Local model",
    localModelLoading: ZH_BROWSER ? "\u6b63\u5728\u8bfb\u53d6\u672c\u5730\u6a21\u578b..." : "Loading local models...",
    ...
    mainTitle: "MiniMax H3 Aicg",
    selectedVideoContextTitle: ZH_BROWSER ? "MiniMax H3 Aicg 已选视频上下文" : "MiniMax H3 Aicg Selected Video Context",
    loaderTitle: ZH_BROWSER ? "MiniMax H3 Aicg \u52a0\u8f7d\u5668" : "MiniMax H3 Aicg Loader",
    adapterTitle: ZH_BROWSER ? "MiniMax H3 Aicg \u6a21\u578b\u4e2d\u8f6c" : "MiniMax H3 Aicg Model Bridge",
    mediaBridgeTitle: ZH_BROWSER ? "MiniMax H3 Aicg \u5a92\u4f53\u4e2d\u8f6c" : "MiniMax H3 Aicg Media Bridge",
    mediaLoaderTitle: ZH_BROWSER ? "MiniMax H3 Aicg\u8d44\u6e90\u5e93" : "MiniMax H3 Aicg Resource Library",
    mediaSplitterTitle: ZH_BROWSER ? "MiniMax H3 Aicg \u5a92\u4f53\u62c6\u5206" : "MiniMax H3 Aicg Media Splitter",
    mediaLoaderMenuTitle: ZH_BROWSER ? "\u8d44\u6e90\u5e93" : "Resource Library",
    mediaLoaderAll: ZH_BROWSER ? "\u7d20\u6750\u8d44\u6e90" : "Asset resources",
    ...
    promptOptimizerSettings: ZH_BROWSER ? "\u6253\u5f00\u63d0\u793a\u8bcd\u4f18\u5316\u8bbe\u7f6e" : "Optimizer settings",
    ...
    segmentRefineTitle: ZH_BROWSER ? "MiniMax H3 Aicg \u5206\u6bb5\u4e8c\u91c7" : "MiniMax H3 Aicg Segment Refine",
    segmentSampleSetupTitle: ZH_BROWSER ? "MiniMax H3 Aicg \u91c7\u6837\u8bbe\u7f6e" : "MiniMax H3 Aicg Sample Setup",
    segmentSampleSetupPort: ZH_BROWSER ? "\u91c7\u6837\u8bbe\u7f6e" : "Sample Setup",
    segmentStepTitle: ZH_BROWSER ? "MiniMax H3 Aicg \u5206\u6bb5\u6b65\u9aa4" : "MiniMax H3 Aicg Segment Step",
    segmentCollectTitle: ZH_BROWSER ? "MiniMax H3 Aicg \u5206\u6bb5\u6c47\u603b" : "MiniMax H3 Aicg Segment Collect",
    segmentDecodeTitle: ZH_BROWSER ? "MiniMax H3 Aicg \u5206\u6bb5\u89e3\u7801" : "MiniMax H3 Aicg Segment Decode",
    contextSegmentsTitle: ZH_BROWSER ? "MiniMax H3 Aicg \u4e0a\u4e0b\u6587\u5206\u6bb5" : "MiniMax H3 Aicg Context Segments",
...
const OPTION_DEFS = {
    ...
    prompt_optimizer_mode: {
        api: ZH_BROWSER ? "\u5728\u7ebf API" : "Online API",
        local: ZH_BROWSER ? "\u672c\u5730\u89c6\u89c9\u6a21\u578b" : "Local vision model",
    },
    ...
    prompt_optimizer_local_device: {
        cuda: "GPU (CUDA)",
        cpu: "CPU",
    },
    prompt_optimizer_output_language: {
        "\u4e2d\u6587": ZH_BROWSER ? "\u4e2d\u6587" : "Chinese",
        English: "English",
    },
...
/* Nodes 2.0 与画布模式都要求「提示词行」有一个可连线输入点。
   把原生 prompt 输入槽指向 DOM 编辑器 widget 后：
   1) 前端 buildSlotMetadata 会按 widget.name 注册槽位，编辑器行左侧出现 dot-only 输入点；
   2) litegraph 的 _arrangeWidgetInputSlots 会把该槽位坐标对齐到编辑器行。
   连线依然落在 inputs[prompt] 上（后端本就支持 prompt 被连线驱动），不需要改后端。 */
function promptEditorSlotWidget(node) {
    const widget = node?.__h3DomWidget;
    if (!widget || !Array.isArray(node?.widgets) || !node.widgets.includes(widget)) return null;
    return widget;
}

function markWidgetSlotLayoutDirty(node) {
    if (!node) return;
...
function bindPromptSlotToEditorWidget(node) {
    const input = promptInputSlot(node);
    const domWidget = promptEditorSlotWidget(node);
    if (!input || !domWidget) return false;
    if (input.widget === domWidget) return true;
    if (!input.__h3NativeWidget) input.__h3NativeWidget = input.widget || getWidget(node, "prompt") || null;
    input.widget = domWidget;
    markWidgetSlotLayoutDirty(node);
    return true;
}

function unbindPromptSlotFromEditorWidget(node) {
    const input = promptInputSlot(node);
    if (!input || !input.__h3NativeWidget) return false;
    if (input.widget !== input.__h3NativeWidget) {
        input.widget = input.__h3NativeWidget;
        input.__h3NativeWidget = null;
        markWidgetSlotLayoutDirty(node);
        return true;
    }
    input.__h3NativeWidget = null;
...
let optimizerDoneAudio = null;

/** 提示词优化完成后的一声轻提示，失败静默，绝不打断流程。 */
function playOptimizerDoneSound() {
    try {
        optimizerDoneAudio?.pause?.();
        optimizerDoneAudio = new Audio(OPTIMIZER_DONE_SOUND_URL);
        optimizerDoneAudio.volume = 0.6;
        optimizerDoneAudio.play?.().catch(() => {});
    } catch {}
}
...
function buildRuntimePrompt(node, runtimeLinks) {
    ...
    // 素材加载器以原生 bundle 接入时没有虚拟连线：@ 引用按加载器内的素材
    // 顺序定位，该顺序与 bundle 的序列化顺序（图 / 音 / 视频）一致。
    const bridgeOptions = runtimeLinks.length ? [] : mentionOptions(node);
    return doc.parts.map((part) => {
        ...
        if (index < 0 && bridgeOptions.length) {
            const mediaIndex = Number(part.mediaIndex);
            if (Number.isFinite(mediaIndex) && mediaIndex > 0 && mediaIndex <= bridgeOptions.length) {
                index = mediaIndex - 1;
            } else {
                const match = bridgeOptions.findIndex((option) =>
                    String(option.type || "image").toLowerCase() === mediaType
                    && (!Number.isFinite(partOrdinal) || Number(option.ordinal) === partOrdinal)
                    && (partSourceId == null || Number(option.sourceId) === partSourceId));
...
function mentionInsertRange(editor) {
    const selection = window.getSelection?.();
    if (selection?.rangeCount) {
        const candidate = selection.getRangeAt(0);
        if (editor.contains(candidate.startContainer) && editor.contains(candidate.endContainer)) return candidate;
    }
    const range = document.createRange();
    range.selectNodeContents(editor);
    range.collapse(false);
...
function chooseMention(node, option) {
    const range = node?.__h3MentionMenu?.mention?.range;
    if (!range || !node?.__h3Editor) return;
    insertMentionOption(node, option, range);
    closeMentionMenu(node);
    node.__h3Editor.focus();
}
...
function setWidgetOption(widget, key, value) {
    ...
    if (key === "hidden" || key === "canvasOnly") scheduleVueWidgetRefresh(widget.node);
}
...
function refreshVueNodeWidgets(node) {
    if (!Array.isArray(node?.widgets) || node.widgets.length === 0) return;
    try {
        if (isVueNodesMode()) {
            // Nodes 2.0：重新赋值数组不一定会触发前端重新映射，pop/push 才会。
...
/* Nodes 2.0 下，改 hidden/canvasOnly 后必须让前端重新读一次 node.widgets，
   否则隐藏不生效（节点上会留下一整块空白）。这里合并到一帧里做。 */
const vueWidgetRefreshQueue = new Set();
let vueWidgetRefreshPending = false;

function scheduleVueWidgetRefresh(node) {
    if (!node || !isVueNodesMode()) return;
    vueWidgetRefreshQueue.add(node);
    if (vueWidgetRefreshPending) return;
    vueWidgetRefreshPending = true;
    const run = () => {
        vueWidgetRefreshPending = false;
        const pending = [...vueWidgetRefreshQueue];
        vueWidgetRefreshQueue.clear();
        for (const item of pending) refreshVueNodeWidgets(item);
    };
...
function syncModeWidgets(node, { adjustHeight = true } = {}) {
    ...
    const changed = [
        ...
        // “提示词方案”不再作为节点上的可见设置项，保留输入是为了兼容已保存的
        // 工作流与 AICG3D 预设（预设仍可直接写入该字段）。
...
async function loadPromptOptimizerLocalModels({ force = false } = {}) {
    if (promptOptimizerLocalModelsPromise && !force) return promptOptimizerLocalModelsPromise;
    if (promptOptimizerLocalModelsCache && !force) return promptOptimizerLocalModelsCache;
    promptOptimizerLocalModelsPromise = (async () => {
        const response = await api.fetchApi(PROMPT_OPTIMIZER_LOCAL_MODELS_ENDPOINT);
        const data = await response.json().catch(() => ({}));
        if (!response.ok || !data?.ok) throw new Error(data?.error || `HTTP ${response.status}`);
        ...
        promptOptimizerLocalModelsPromise = null;
    });
    return promptOptimizerLocalModelsPromise;
}
...
function setPromptOptimizerSettingsToggle(node, enabled) {
    ...
    if (widget._state) widget._state.value = value;
    node?.setDirtyCanvas?.(true, true);
    // Nodes 2.0 的开关只在 widget 列表刷新后重绘，而刷新不能发生在组件自身
    // 的点击回调里，因此延后一帧执行。
    const refresh = () => refreshVueNodeWidgets(node);
    if (typeof requestAnimationFrame === "function") requestAnimationFrame(refresh);
    else refresh();
}

function resetPromptOptimizerSettingsToggle(node) {
    setPromptOptimizerSettingsToggle(node, false);
}
...
function makePromptOptimizerChoice(initialValue, optionDef, ariaLabel) {
    const root = document.createElement("div");
    root.className = "h3-optimizer-settings-choice";
    root.setAttribute("role", "radiogroup");
    if (ariaLabel) root.setAttribute("aria-label", ariaLabel);
    const entries = Object.entries(optionDef || {});
    const values = entries.map(([value]) => value);
    const buttons = [];
    let current = values.includes(initialValue) ? initialValue : values[0] || "";
    const render = () => {
    ...
    for (const [value, label] of entries) {
        ...
        button.type = "button";
        button.className = "h3-optimizer-settings-choice-option";
...
async function openPromptOptimizerSettings(node) {
    const openModal = promptOptimizerSettingsModal;
    if (openModal?.dialog?.isConnected) {
        openModal.dialog.querySelector?.("input, button")?.focus?.();
        return;
    }
    // 弹窗可能已被外部移除（切换工作流、刷新界面等）。残留的单例状态会让后续
    // 点击直接提前返回，表现为“提示词优化设置打不开”，所以先清理再重建。
    if (openModal) {
        openModal.overlay?.remove?.();
        promptOptimizerSettingsModal = null;
    }
    if (promptOptimizerSettingsOpening) {
        await promptOptimizerSettingsOpening.catch(() => {});
        return;
    }
    promptOptimizerSettingsOpening = (async () => {
        // 读取设置失败或后端响应过慢时同样要弹出面板，否则点击开关看起来毫无反应。
        const loading = loadPromptOptimizerSettings().catch((error) => {
        ...
        try {
            buildPromptOptimizerSettingsDialog(node);
        } catch (error) {
            promptOptimizerSettingsModal = null;
            document.querySelector(".h3-optimizer-settings-overlay")?.remove?.();
            notifyPromptOptimizer(error?.message || TEXT.settingsLoadFailed);
        }
    })();
    try {
        await promptOptimizerSettingsOpening;
    } finally {
        promptOptimizerSettingsOpening = null;
    }
}

/* 面板刚打开的这一小段时间里，受控开关组件重绘时可能回传旧值，忽略这种“假关闭”。 */
function promptOptimizerSettingsJustOpened() {
    return Boolean(
        promptOptimizerSettingsModal?.dialog?.isConnected
        && promptOptimizerSettingsOpenedAt
        && Date.now() - promptOptimizerSettingsOpenedAt < 400
    );
}

/* Nodes 2.0 下这一行由 Vue 组件渲染，元素会在重绘时被替换、widget.callback 也不保证被调用，
   因此在 DOM 上补一层兜底：点击开关或它所在的那一行都直接打开设置面板。 */
const PROMPT_OPTIMIZER_TOGGLE_SELECTOR = '[aria-label="prompt_optimizer_settings"]';
const PROMPT_OPTIMIZER_UI_BUILD = "按钮是入口非开关";
let promptOptimizerToggleBridgeAt = 0;

function promptOptimizerLabelTexts() {
    return [
        TEXT.promptOptimizerSettings,
        "打开提示词优化设置",
        "Optimizer settings",
    ].map((item) => String(item || "").trim()).filter(Boolean);
}

function isPromptOptimizerRowLabel(element) {
    const text = String(element?.textContent || "").trim();
    if (!text) return false;
    return promptOptimizerLabelTexts().some((label) => text === label || text.startsWith(label));
}

function promptOptimizerGraphNodes() {
    const graph = globalThis.app?.graph;
    return (graph?._nodes || graph?.nodes || []).filter(
        (item) => item && getWidget(item, "prompt_optimizer_settings")
    );
}

function promptOptimizerNodeFromElement(element) {
    const graph = globalThis.app?.graph;
    const rawId = element?.closest?.("[data-node-id]")?.getAttribute?.("data-node-id");
    if (rawId) {
        const numeric = Number(rawId);
        const byId = Number.isFinite(numeric) ? graph?.getNodeById?.(numeric) : null;
        const direct = byId?.widgets
            ? byId
            : graph?._nodes_by_id?.[rawId]
                || (graph?.nodes || []).find((item) => String(item?.id) === String(rawId));
        if (direct) return direct;
    }
    // 兜底：节点 DOM 不一定挂在 data-node-id 里，按节点根元素归属再找一次。
    const candidates = promptOptimizerGraphNodes();
    if (candidates.length === 1) return candidates[0];
    for (const node of candidates) {
        const root = document.querySelector(`[data-node-id="${node.id}"]`);
        if (root?.contains?.(element)) return node;
    }
    const selected = globalThis.app?.canvas?.selected_nodes || {};
    return candidates.find((node) => selected[node.id] || selected[String(node.id)])
        || candidates[0]
        || null;
}

function promptOptimizerToggleChecked(toggle, node) {
    if (toggle?.getAttribute?.("aria-checked") === "true") return true;
    if (toggle?.dataset?.state === "checked") return true;
    return asBoolean(getWidget(node, "prompt_optimizer_settings")?.value, false);
}

function promptOptimizerToggleIn(element) {
    if (!element?.querySelector) return null;
    return element.querySelector(PROMPT_OPTIMIZER_TOGGLE_SELECTOR)
        || element.querySelector('[role="switch"], [aria-checked], [data-state]');
}

function resolvePromptOptimizerToggleHit(target) {
    if (!target?.closest) return null;
    const toggle = target.closest(PROMPT_OPTIMIZER_TOGGLE_SELECTOR);
    if (toggle) {
        const row = toggle.closest('[data-testid="node-widget"]') || toggle.parentElement || toggle;
        return { row, toggle };
    }
    // 紧凑/折叠显示时控件位置会变，退化为“文字 + 开关”识别整行。
    let element = target;
    for (let depth = 0; element && depth < 8; depth += 1, element = element.parentElement) {
        if (element.hasAttribute?.("data-node-id")) break;
        if (!isPromptOptimizerRowLabel(element)) continue;
        const switchEl = promptOptimizerToggleIn(element);
...
function handlePromptOptimizerToggleDomEvent(event) {
    const hit = resolvePromptOptimizerToggleHit(event?.target);
    if (!hit) return;
    const node = promptOptimizerNodeFromElement(hit.row);
    if (!node || !getWidget(node, "prompt_optimizer_settings")) return;
    if (promptOptimizerSettingsModal?.dialog?.isConnected) {
        if (promptOptimizerToggleChecked(hit.toggle, node)) promptOptimizerSettingsModal?.close?.();
        else openPromptOptimizerSettings(node);
        return;
    }
    setPromptOptimizerSettingsToggle(node, true);
    openPromptOptimizerSettings(node);
    // 点了却打不开时给个可见提示，用来区分“前端还在跑旧代码”和“面板被别的层挡住”。
    setTimeout(() => {
        if (!promptOptimizerSettingsModal?.dialog?.isConnected) {
            notifyPromptOptimizer(`${TEXT.promptOptimizerSettings} 面板未打开（UI ${PROMPT_OPTIMIZER_UI_BUILD}）`);
        }
    }, 1800);
}

function installPromptOptimizerToggleDomBridge() {
    if (globalThis.__h3OptimizerToggleBridgeInstalled) return;
    globalThis.__h3OptimizerToggleBridgeInstalled = true;
    const handler = (event) => {
        // 同一次点击会依次产生 pointerdown / click，只处理第一次。
        const now = Date.now();
        if (now - promptOptimizerToggleBridgeAt < 350) return;
        try {
            if (!resolvePromptOptimizerToggleHit(event?.target)) return;
            promptOptimizerToggleBridgeAt = now;
            handlePromptOptimizerToggleDomEvent(event);
        } catch { /* 兜底监听不能影响正常点击 */ }
    };
...
function buildPromptOptimizerSettingsDialog(node) {
    ...
    const apiFormatRow = makePromptOptimizerSettingsRow(TEXT.apiFormat, apiFormat);
    ...
    const localDevice = makePromptOptimizerChoice(
        promptOptimizerSettingsCache.local_device,
        OPTION_DEFS.prompt_optimizer_local_device,
        TEXT.localDevice
    );
    const localDeviceRow = makePromptOptimizerSettingsRow(TEXT.localDevice, localDevice);
    const localDeps = document.createElement("div");
    ...
    const outputLanguageRow = makePromptOptimizerSettingsRow(TEXT.outputLanguage, outputLanguage);
    ...
    const applyLocalModels = (payload) => {
        ...
        for (const item of localModels) {
            ...
            if (value) defs[value] = `${String(item?.name || value)}${item?.format === "gguf" ? " \u00b7 GGUF" : ""}`;
        }
        if (!localModels.length) defs[""] = TEXT.localModelEmpty;
        localModelSelect.setOptions(defs, promptOptimizerSettingsCache.local_model);
        const missing = Array.isArray(payload?.missingDependencies) ? payload.missingDependencies : [];
        localDeps.dataset.missing = missing.length ? "1" : "0";
        localDeps.textContent = missing.length ? `${TEXT.localDepsMissing}${missing.join(", ")}` : "";
        localDeps.dataset.missing = missing.length ? "1" : "0";
        syncEngineVisibility();
    };
    localModelSelect.addEventListener("change", syncMmprojRow);
    loadPromptOptimizerLocalModels()
        .then(applyLocalModels)
        .catch(() => {
            localModelSelect.setOptions({ "": TEXT.localModelFailed }, "");
            mmprojRow.hidden = true;
        });
    ...
    const syncEngineVisibility = () => {
        const local = engineSelect.value === PROMPT_OPTIMIZER_ENGINE_LOCAL;
        apiFormatRow.hidden = local;
        apiUrlRow.hidden = local;
        apiKeyRow.hidden = local;
        modelRow.hidden = local;
        localModelRow.hidden = !local;
        localDeviceRow.hidden = !local;
        localDeps.hidden = !local || localDeps.dataset.missing !== "1";
        ollamaUnloadLabel.hidden = local ? false : apiFormat.value !== "ollama";
        syncMmprojRow();
    };
    apiFormat.addEventListener("change", syncEngineVisibility);
    ...
    const close = () => {
        ...
        overlay.remove();
        promptOptimizerSettingsModal = null;
        promptOptimizerSettingsOpenedAt = 0;
    ...
    promptOptimizerSettingsModal = { dialog, close, overlay };
    promptOptimizerSettingsOpenedAt = Date.now();
    ...
    overlay.addEventListener("pointerdown", (event) => {
        for (const select of [engineSelect, apiFormat, localModelSelect, mmprojSelect]) {
            if (!select.contains?.(event.target)) select.__h3CloseMenu?.();
        }
    ...
    form.addEventListener("submit", async (event) => {
        ...
        try {
            ...
            await savePromptOptimizerSettings({
                ...
                local_mmproj: mmprojSelect.value,
...
function promptOptimizerConfigured(state) {
    if (String(state?.mode || PROMPT_OPTIMIZER_ENGINE_API) === PROMPT_OPTIMIZER_ENGINE_LOCAL) {
        return Boolean(String(state?.local_model || "").trim());
    }
    return Boolean(
        state?.api_url?.trim()
        && state?.model?.trim()
    ...
    const worker = async () => {
        while (true) {
            ...
            const data = await requestPromptOptimization({
                ...commonPayload,
                resources: filterOptimizerResources(commonPayload.resources || [], segments[index]),
...
/** 把 <Picture 3> / <Video 1> / <Audio 2> 之类的标签规范化，便于比对。 */
function normalizeMediaTag(value) {
    const match = /^<\s*(picture|video|audio)\s+(\d+)\s*>$/i.exec(String(value || "").trim());
    return match ? `<${match[1].toLowerCase()} ${Number(match[2])}>` : String(value || "").trim().toLowerCase();
}

/** 提示词里实际写到的素材标签集合。 */
function referencedMediaTags(text) {
    const tags = new Set();
    const pattern = /<\s*(picture|video|audio)\s+(\d+)\s*>/gi;
    let match = pattern.exec(String(text || ""));
    while (match) {
        tags.add(normalizeMediaTag(match[0]));
...
/**
 * AICG3D资源库里挂着多少素材，就只把提示词真正引用到的那些交给优化器。
 * 例如 7 张图里只写了 <Picture 1> 与 <Picture 4>，其余图片不参与请求，
 * 模型就不会把它们统统写进 subject_definitions / retention_analysis。
 * 提示词完全没有引用时保持原行为（全部下发）。
 */
function filterOptimizerResources(resources, prompt) {
    const referenced = referencedMediaTags(prompt);
    if (!referenced.size) return resources;
    const kept = resources.filter((item) => referenced.has(normalizeMediaTag(item?.tag)));
    return kept.length ? kept : resources;
}
...
async function optimizePromptFromEditor(node) {
    ...
    if (!promptOptimizerConfigured(state)) {
        notifyPromptOptimizer(
            String(state?.mode || PROMPT_OPTIMIZER_ENGINE_API) === PROMPT_OPTIMIZER_ENGINE_LOCAL
    ...
    const resources = filterOptimizerResources(promptOptimizerResources(node), sourcePrompt);
...
function ensurePromptEditor(node) {
    ...
    if (canUseMediaMentions(node)) {
        ...
        skillButton.title = TEXT.aicg3dSkills;
        skillButton.setAttribute("aria-label", TEXT.aicg3dSkills);
        ...
        skillButton.addEventListener("click", (event) => {
            event.preventDefault();
            event.stopPropagation();
            globalThis.AICG3D?.openSkillPalette?.(node, skillButton);
        });
        tools.push(skillButton);
    }
...
function removePromptEditorWidgets(node) {
    ...
    unbindPromptSlotFromEditorWidget(node);
        ...
        widget.callback = (value) => {
            if (name === "prompt_optimizer_settings") {
                ...
                if (!wantsOpen && promptOptimizerSettingsJustOpened()) {
                    // 组件重绘可能回传旧值，忽略这次关闭，避免面板刚打开就被收起。
                    setPromptOptimizerSettingsToggle(node, true);
                    return;
                }
                setPromptOptimizerSettingsToggle(node, wantsOpen);
                if (wantsOpen) openPromptOptimizerSettings(node);
                else promptOptimizerSettingsModal?.close?.();
                node.setDirtyCanvas?.(true, true);
                return;
            }
...
function mediaLoaderCreateCard(node, group, filename) {
    ...
    // AICG3D：单击素材卡片 = 把该素材 @ 进主节点提示词。
    card.addEventListener("click", (event) => {
        if (card.classList.contains("is-dragging")) return;
        if (event.target?.closest?.(".h3-media-loader-remove")) return;
        const index = Number(card.dataset.index);
        if (!Number.isInteger(index)) return;
        aicg3dInsertLoaderMention(node, group.type, index);
    });
...
function installMediaLoaderNode(nodeType, nodeData) {
    ...
    nodeType.prototype.onConfigure = function onConfigureH3MediaLoader(info) {
        const result = originalConfigure?.apply(this, arguments);
        setup(this);
        // A saved workflow restores its own input/output labels, which would
        // otherwise pin the socket to whatever name it had when it was saved.
        localizeNodeInstance(this);
        mediaLoaderRender(this);
        return result;
    };
...
function install() {
    ...
    installPromptOptimizerToggleDomBridge();
    ...
    style.textContent = `
      ...
      dialog.h3-optimizer-settings-overlay {
        margin: 0; width: auto; height: auto; max-width: none; max-height: none; border: 0; display: flex;
      }
      ...
      .h3-optimizer-settings-note[hidden] { display: none !important; }
      .h3-optimizer-settings-choice {
        display: flex; gap: 4px; padding: 3px; border: 1px solid var(--h3-settings-border); border-radius: 9px; background: var(--h3-settings-bg-base);
      }
      .h3-optimizer-settings-choice-option {
        flex: 1 1 0; min-width: 0; height: 26px; padding: 0 10px; border: 0; border-radius: 6px; background: transparent; color: var(--h3-settings-muted);
        cursor: pointer; font: inherit; font-size: 13px; font-weight: 500; transition: background .18s, color .18s;
      }
      .h3-optimizer-settings-choice-option:hover { background: var(--h3-settings-bg-hover); color: var(--h3-settings-text); }
      .h3-optimizer-settings-choice-option.is-selected { background: rgba(168,199,250,.16); color: #dce7fa; font-weight: 600; }
...
/* ==========================================================================
   AICG3D 桥接层
   素材加载器 / 提示词编辑器暴露给 AICG3D 公共前端（素材库、技能库）的接口。
   ========================================================================== */
...
function aicg3dAppendPlainText(node, text) {
    ...
    if (!editor) {
        const widget = getWidget(node, "prompt");
        if (!widget) return false;
        widget.value = `${String(widget.value || "")}${text}`;
        return true;
    }
    // 通过编辑器插入并回写，结构化与原始视图都能正确落到 prompt widget。
    placeCaretAtEditorEnd(editor);
    insertPlainText(editor, text);
    syncPromptFromEditor(node);
    pushPromptHistory(node);
...
/** 素材加载器里的第 index 个素材 -> 主节点的 @ 引用。 */
function aicg3dInsertLoaderMention(loader, type, index) {
    const ordinal = Number(index) + 1;
    if (!Number.isFinite(ordinal) || ordinal < 1) return false;
    const targets = aicg3dPromptTargets(loader);
    if (!targets.length) {
        globalThis.alert?.(TEXT.aicg3dNoTarget);
        return false;
    }
...
globalThis.AICG3D_H3 = {
    ...
    async insertSkillText(node, skill) {
        ...
        let body = "";
        try {
            const core = await import("./aicg3d-core.js");
            body = String((await core.library.detail(id))?.body || "");
        } catch (error) {
            console.error("[AICG3D] 读取技能失败", error);
            return false;
        }
        ...
        const clipped = body.length > limit
            ? `${body.slice(0, limit)}\n……（正文过长已截断，完整内容见技能库）`
            : body;
        return aicg3dAppendPlainText(node, `\n【技能：${name}】\n${clipped}\n【技能结束】\n`);
    },

    /** 从输入目录挑中的素材：补进素材加载器后立即 @ 引用。 */
    attachMediaFromLibrary(node, file) {
        const loader = aicg3dLinkedLoader(node);
        if (!loader) return false;
        const group = MEDIA_LOADER_GROUPS.find((entry) => entry.type === file?.kind);
        if (!group) return false;
        const state = mediaLoaderReadState(loader);
        const list = state[group.key];
        if (!list.includes(file.path)) {
            list.push(file.path);
            mediaLoaderWriteState(loader, state);
            mediaLoaderRender(loader);
            mediaLoaderResize(loader);
        }
        return aicg3dInsertLoaderMention(loader, group.type, list.indexOf(file.path));
    },

    /** 上传到 ComfyUI 输入目录 -> 加入素材加载器 -> 插入 @ 引用。 */
    async uploadToTarget(node, files) {
        const loader = aicg3dLinkedLoader(node);
        if (!loader) return 0;
        const state = mediaLoaderReadState(loader);
        const added = [];
        for (const file of Array.from(files || [])) {
            ...
            try {
                const filename = await mediaLoaderUploadOne(file);
                if (!filename || state[group.key].includes(filename)) continue;
                state[group.key].push(filename);
                added.push({ type: group.type, index: state[group.key].length - 1 });
            } catch (error) {
                console.error("[AICG3D] 上传失败", error);
            }
        }
        ...
        mediaLoaderRender(loader);
        mediaLoaderResize(loader);
        for (const entry of added) aicg3dInsertLoaderMention(loader, entry.type, entry.index);
        return added.length;
    },
};
