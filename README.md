...
### 28. 下拉列表宽度不跟节点走，长文件名被截断（已修）

症状：把「MiniMax H3 Aicg 加载器」拉宽之后点开 LoRA 下拉，列表还是原来那么窄，
`minimax\风格\lora\H3_Combat_V2强化格斗…` 这种带目录的长名字被省略号截掉，看不全。

原因：下拉框本身是跟着节点变宽的（`.a3-select { width: 100% }`），但**弹出的列表**宽度在
`web/aicg3d-core.js` 里写死了一个 `Math.min(320, rect.width + 40)` 的上限，节点再宽列表也停在 320px。

现在列表宽度跟着下拉框走：`触发框宽 + 40px`，下限 220px、上限 900px，再按屏幕边距收口。
节点拉多宽列表就多宽。所有用 `a3-select` 的下拉（LoRA / 模型 / 文本编码器 / VAE / 提示词方案）
一起生效。

纯前端改动，刷新浏览器（F5）即可，不用重启 ComfyUI。
