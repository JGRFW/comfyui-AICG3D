...
const GH_HIDDEN_WIDGETS = [
    "main_mode", "prompt", "task_type", "audio_mode", "audio_denoise_strength",
    "drive_audio_ordinal", "ref_image_size", "strict_prompt_tags", "gh_state_json",
    ...mediaSlots,
];
...
const DOM_TRANSLATIONS = {
    ...
    "Supports up to 3 videos, 9 images, and 3 audios · Only media you @-mention in the prompt are sent · Video MP4/MOV (2-30 seconds) · Audio MP3/WAV (2-30 seconds)": "支持视频x3，图片x9，音频x3 · 只发送提示词里 @ 到的素材 · 视频MP4/MOV(2-30秒) · 音频MP3/WAV(2-30秒)",
...
function hideWidget(w) {
    if (!w) return;
    w.hidden = true; w.options = w.options || {};
    w.options.hidden = true; w.options.canvasOnly = true;
    if (w._state?.options) { w._state.options.hidden = true; w._state.options.canvasOnly = true; }
    w.computeSize = () => [0, -4]; w.serialize = true;
    refreshVueWidgets(w.node);
}
...
function nodeColorToCss(value) {
    ...
    function addDrop(slot = null, referenceEmpty = false) {
        ...
        else if (referenceEmpty) { title = t("Reference media"); subtitle = t("Supports up to 3 videos, 9 images, and 3 audios · Only media you @-mention in the prompt are sent · Video MP4/MOV (2-30 seconds) · Audio MP3/WAV (2-30 seconds)"); }
...
app.registerExtension({
    ...
    autoHideWidgets(nodeType, GH_HIDDEN_WIDGETS, hideWidget);
