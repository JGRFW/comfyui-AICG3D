...
PROMPT_OPTIMIZER_ENGINES = ("api", "local")
PROMPT_OPTIMIZER_LANGUAGES = ("中文", "English")
PROMPT_OPTIMIZER_LOCAL_DEVICES = ("cuda", "cpu")
PROMPT_OPTIMIZER_LANGUAGE_SECTIONS = {
    "zh": (
        "\n\n=== OUTPUT LANGUAGE ===\n"
        "Write every descriptive sentence of the final optimized prompt in Simplified Chinese. "
        "Keep the official H3 field names, section headers, timing notation, dialogue inside <d>...</d>, "
        "and reference tags exactly as defined by the Prompt Guide; never translate, rename, or reorder them."
    ),
    "en": (
        "\n\n=== OUTPUT LANGUAGE ===\n"
        "Write every descriptive sentence of the final optimized prompt in English. "
        "Keep the official H3 field names, section headers, timing notation, dialogue inside <d>...</d>, "
        "and reference tags exactly as defined by the Prompt Guide; never translate, rename, or reorder them."
    ),
}
...
def _collect_weight_names(categories: tuple[str, ...]) -> list[str]:
    ...
    result cached made the Aicg Loader retain the first snapshot for the
...
def _normalize_prompt_optimizer_config(value: Mapping[str, Any] | None) -> dict[str, Any]:
    ...
    engine = str(source.get("mode") or PROMPT_OPTIMIZER_ENGINES[0]).strip().lower()
    if engine not in PROMPT_OPTIMIZER_ENGINES:
        engine = PROMPT_OPTIMIZER_ENGINES[0]
    language = str(source.get("output_language") or DEFAULT_PROMPT_OPTIMIZER_LANGUAGE).strip().lower()
    output_language = "English" if language in {"english", "en"} else DEFAULT_PROMPT_OPTIMIZER_LANGUAGE
    local_device = str(source.get("local_device") or "cuda").strip().lower()
    if local_device not in PROMPT_OPTIMIZER_LOCAL_DEVICES:
        local_device = "cuda"
    return {
        "version": PROMPT_OPTIMIZER_CONFIG_VERSION,
        "mode": engine,
        ...
        "local_mmproj": str(source.get("local_mmproj") or "").strip(),
...
def _prompt_optimizer_mode(settings: Mapping[str, Any] | None) -> str:
    """Return the active optimization engine: the online API or a local VLM."""
    engine = str((settings or {}).get("mode") or PROMPT_OPTIMIZER_ENGINES[0]).strip().lower()
    return engine if engine in PROMPT_OPTIMIZER_ENGINES else PROMPT_OPTIMIZER_ENGINES[0]


def _prompt_optimizer_language_key(settings: Mapping[str, Any] | None) -> str:
    value = str((settings or {}).get("output_language") or "").strip().lower()
...
def _prompt_optimizer_ready(settings: Mapping[str, Any] | None) -> bool:
    """True when the stored settings can actually run one optimization request."""
    source = settings if isinstance(settings, Mapping) else {}
    if _prompt_optimizer_mode(source) == "local":
        return bool(str(source.get("local_model") or "").strip())
    return _prompt_optimizer_settings_complete(
        str(source.get("api_url") or ""),
        str(source.get("api_key") or ""),
        str(source.get("model") or ""),
        str(source.get("api_format") or "openai"),
    )
...
def _unload_ollama_model(api_url: str, model: str, timeout_seconds: float) -> None:
    try:
        ...
        print(f"[MiniMax H3 Aicg] Ollama model unload skipped: {exc}")
...
def _optimizer_local_resources(
    resources: list[Mapping[str, Any]],
    maximum: int = MAX_MEDIA,
) -> list[tuple[Mapping[str, Any], str, str]]:
    """Keep the connected assets the local engine can actually attach."""
    ...
    for resource in resources[: max(0, int(maximum))]:
        ...
        if not path:
            continue
        selected.append((resource, media_type, path))
    return selected


def _optimizer_media_resource_count(
    resources: list[Mapping[str, Any]],
    maximum: int = MAX_MEDIA,
) -> int:
    """Count attached assets without paying for any base64 encoding."""
    return len(_optimizer_local_resources(resources, maximum))


def _optimizer_local_media(
    resources: list[Mapping[str, Any]],
    maximum: int = MAX_MEDIA,
) -> list[dict[str, Any]]:
    """Encode connected assets once, in the shape the local engine expects."""
    media: list[dict[str, Any]] = []
    for resource, media_type, path in _optimizer_local_resources(resources, maximum):
        label = _optimizer_resource_tag(resource) or str(resource.get("name") or media_type)
        images: list[str] = []
        if media_type == "image":
            try:
                ...
                images = []
        media.append({"label": label, "kind": media_type, "images": images})
    return media


def _optimizer_local_generate(
    ...
    try:
        ...
        raise ValueError("Goohai 模块不可用，请确保插件完整安装")
    if not local_model:
        raise ValueError("请先在提示词优化设置中选择本地视觉模型")
    config = {
        "mode": "local",
        "output_language": str(settings.get("output_language") or DEFAULT_PROMPT_OPTIMIZER_LANGUAGE),
        "local_model": local_model,
        "local_mmproj": str(settings.get("local_mmproj") or "").strip(),
...
def _optimizer_prompt_has_references(prompt: str) -> bool:
    """提示词里是否真的写了素材引用（<Picture N>/<Video N>/<Audio N>）。"""
    return bool(REFERENCE_TAG_PATTERN.search(str(prompt or "")))


def _optimizer_attached_resources(
    resources: list[Mapping[str, Any]],
    read_media: bool,
    prompt: str,
    maximum: int = MAX_MEDIA,
) -> list[Mapping[str, Any]]:
    """只有开着“读取已连接媒体”、且提示词确实引用了素材时才附带附件。

    否则模型会把素材包里的东西写成提示词里的引用，而引用会真的改变运行时绑定的素材。
    """
    selected = list(resources or [])[: max(0, int(maximum))]
    if not read_media or not _optimizer_prompt_has_references(prompt):
        return []
    if not _optimizer_media_read_allowed(selected):
        return []
    return selected
...
def _optimizer_system_prompt(
    ...
    # 素材只是“看懂画面”的证据：模型不能替用户新增素材引用，否则运行时会真的多绑定素材。
    if has_media_references:
        prompt += (
            "\n\n=== MEDIA REFERENCE DISCIPLINE ===\n"
            "Reference only the media the user's original prompt already references. "
        ...
        prompt += (
            ...
            "The user's original prompt contains no media reference, so no media is bound for this request and this is a plain text-to-video prompt. "
            "Ignore every earlier instruction that asks for reference labels or a subject_definitions section. "
            "Return one plain MiniMax H3 video prompt: no <Picture N>/<Video N>/<Audio N> tags, no subject_definitions, no retention_analysis, "
...
def _runtime_optimizer_context_hash(
    ...
    payload = {
        ...
        "engine": _prompt_optimizer_mode(settings),
...
def _optimizer_context_segment_call(
    *,
    ...
    engine_settings: dict[str, Any] = dict(settings) if isinstance(settings, Mapping) else {}
    if not engine_settings:
        engine_settings = {
            "mode": PROMPT_OPTIMIZER_ENGINES[0],
            "api_format": api_format,
            "unload_ollama_after_optimize": unload_ollama_after_optimize,
        }
    local_engine = _prompt_optimizer_mode(engine_settings) == "local"
    # 提示词没有真正引用素材时不附带附件，否则模型会把素材包里的东西写成引用。
    attached_resources = _optimizer_attached_resources(
        selected_resources,
        read_media,
        resolved_prompt,
        CONTEXT_PROMPT_OPTIMIZER_MEDIA_MAX_RESOURCES,
    )
    media_parts = [] if local_engine else _optimizer_media_parts(
        attached_resources, api_format, CONTEXT_PROMPT_OPTIMIZER_MEDIA_MAX_RESOURCES
    )
    attached_media_count = (
        _optimizer_media_resource_count(attached_resources, CONTEXT_PROMPT_OPTIMIZER_MEDIA_MAX_RESOURCES)
        if local_engine
    ...
    optimized = _optimizer_request(
        ...
        api_format=api_format,
        system=system,
        user_prompt=_optimizer_single_segment_user_prompt(resolved_prompt, previous_prompts),
    ...
    with ThreadPoolExecutor(max_workers=worker_count, thread_name_prefix="h3-prompt") as executor:
        ...
        for future in as_completed(futures):
            try:
                ...
                print(f"[MiniMax H3 Aicg] Context segment prompt optimization skipped: {exc}")
    return "\n\n---\n\n".join(results)
...
def _optimize_prompt_on_run(
    ...
    try:
        ...
        # 否则模型会把素材包里的东西写成提示词里的引用，而引用会真的改变运行时绑定的素材。
        attached_resources = _optimizer_attached_resources(
            resources, read_media, request_prompt, resource_limit
        )
        media_parts = [] if local_engine else _optimizer_media_parts(
            attached_resources, api_format, resource_limit
        )
        attached_media_count = (
            _optimizer_media_resource_count(attached_resources, resource_limit)
            if local_engine
            else len(media_parts)
        )
        system = _optimizer_system_prompt(
...
def _register_prompt_optimizer_route() -> bool:
    ...
    @routes.get("/minimax_h3_easy/prompt_optimizer_local_models")
    async def _prompt_optimizer_local_models_get(request):
        ...
        try:
            ...
            return web.json_response({
                "ok": True,
                "models": await asyncio.to_thread(goohai_optimizer.scan_local_models),
                "missing_dependencies": goohai_optimizer.missing_local_dependencies(),
                "gguf_dependency": goohai_optimizer.gguf_dependency_status(),
            })
        except ImportError:
            return web.json_response({"ok": False, "error": "Goohai module not available"}, status=503)
        except Exception as exc:
            return web.json_response({"ok": False, "error": str(exc)}, status=500)
    ...
    async def _prompt_optimize(request):
        try:
            ...
            if not prompt.strip() or not _prompt_optimizer_ready(settings):
...
def _lora_choices() -> list[str]:
    names = _sort_model_names(list(folder_paths.get_filename_list("loras")))
    return [*names, *NONE_MODEL_DISPLAY_VALUES]


def _apply_lora_stack(model: Any, stack: tuple[tuple[str, float], ...]) -> Any:
    """按顺序叠加 LoRA。权重列表为空时原样返回。"""
    if model is None or not stack:
        return model
    loader = nodes.LoraLoaderModelOnly()
    patched = model
    for name, strength in stack:
        patched, = loader.load_lora_model_only(patched, name, strength)
    return patched
...
class MiniMaxH3Bundle:
    ...
    def __post_init__(self) -> None:
        ...
        self._model_kind = ""
        self._model_name = ""
        self._lora_stack: tuple[tuple[str, float], ...] = ()
        self._patched_models: dict[int, tuple[tuple, Any]] = {}
        self._lock = threading.RLock()

    def _lora_signature(self) -> tuple[tuple[str, float], ...]:
        return tuple(
            (str(name), round(float(strength), 4))
            for name, strength in self.loras or ()
            if name and not _is_none_model(name)
        )

    def _with_loras(self, model: Any) -> Any:
        """给已经加载好的 transformer 叠加当前 LoRA 组合。

        外部传入的 MODEL（Model Adapter 路径）会被复用，所以按对象缓存一次
        结果，避免同一次运行里把同一条 LoRA 重复打上去。
        """
        stack = self._lora_stack
        if model is None or not stack:
            return model
        key = id(model)
        cached = self._patched_models.get(key)
        if cached is not None and cached[0] == stack and cached[1] is not None:
            return cached[1]
...
class MiniMaxH3Context:
    ...
    fps: float
    aspect_ratio: str
    # AICG3D：主节点输出收敛成单条 MINIMAX_H3_CONTEXT 之后，采样需要的 model
    # 随 context 一起传递，画布上不再需要额外拉一条 MODEL 线。
    model: Any = None
    ...
    # context node. Existing Aicg/Context Segments contexts leave this unset,
    # so the field remains backwards-compatible with older workflows.
    selected_video: Any = None


def _resolve_h3_model(h3_context: Any, model: Any = None) -> Any:
    """采样用的 model：显式连线优先，否则回落到 context 自带的 model。"""
    if model is not None:
        return model
    if isinstance(h3_context, MiniMaxH3Context) and h3_context.model is not None:
        return h3_context.model
    raise ValueError(
        "Connect the H3 Context output of a MiniMax H3 Aicg node, or wire its model "
        "output (through MiniMax H3 Aicg Output) into the model input."
    )
...
def _sync_reference_video_cache_scope(items: list[_MediaInput]) -> None:
    """Keep only videos used by the current Aicg run."""
    ...
    if removed:
        if not current:
            print(f"[MiniMax H3 Aicg] Reference video decode cache cleared: {removed} old video(s)")
        else:
            print(f"[MiniMax H3 Aicg] Reference video decode cache switched: cleared {removed} old video(s)")
    ...
    if hasattr(value, "get_components"):
        ...
        if key is not None:
            ...
            if cached is not None:
                print(f"[MiniMax H3 Aicg] Reference video decode cache hit: {label}")
                return cached.frames, cached.audio, cached.fps
            if invalidated:
                print(f"[MiniMax H3 Aicg] Reference video decode cache invalidated: {label}")
            print(f"[MiniMax H3 Aicg] Reference video decode cache miss: {label}")
        ...
        if key is not None and isinstance(frames, torch.Tensor):
            ...
            if _store_reference_video_cache_entry(key, entry):
                print(f"[MiniMax H3 Aicg] Reference video decode cache stored: {label}")
            else:
                print(
                    "[MiniMax H3 Aicg] Reference video decode cache skipped-too-large: "
...
def _expand_unresolved_reference_tags(prompt: str) -> str:
    """把前端没能定位到具体槽位的 @ 引用补成官方标签。

    前端在无法匹配素材槽位时会留下 __MINIMAX_H3_UNRESOLVED_REF_<type>__，
    这里按出现顺序回填 <Picture/Video/Audio N>，避免内部标记泄漏进提示词，
    同时保住用户的引用意图。
    """
    ...
    def replace(match: re.Match) -> str:
        ...
        return f"<{prefixes[media_type]} {counters[media_type]}>"

    return UNRESOLVED_REFERENCE_RE.sub(replace, str(prompt or ""))
...
def _resolve_reference_prompt(
    ...
    resolved = _expand_unresolved_reference_tags(resolved)
...
def bind_reference_media(text: str, items: list[_MediaInput]) -> tuple[list[_MediaInput], str]:
    """Narrow one reference call to the media its prompt actually mentions.

    The Media Loader is an unrestricted library, while a single H3 reference
    call only accepts 9 pictures, 3 videos and 3 audio clips. Prompts that
    mention media are therefore reduced to those inputs and renumbered
    compactly so the tags in the prompt and the references handed to H3 stay
    aligned. Prompts without any mention keep every input so existing
    workflows behave exactly as before.
    """
    source = str(text or "")
    if not items:
        return list(items), source
    if not SEGMENT_TAG_PATTERN.search(_segment_expand_media_placeholders(source, items)):
        return list(items), source
    chosen, rewritten = bind_segment_media(source, items)
    if not chosen:
        return list(items), source
    return chosen, rewritten
...
def _validate_reference_media(items: list[_MediaInput], scope: str = "Reference mode") -> None:
    ...
    if len(items) > MAX_MEDIA:
        raise ValueError(f"{scope} accepts at most {MAX_MEDIA} media resources, but received {len(items)}")
    counts = {"image": 0, "video": 0, "audio": 0}
    for item in items:
    ...
    if counts["image"] > MAX_IMAGES or counts["video"] > MAX_VIDEOS or counts["audio"] > MAX_AUDIOS:
        raise ValueError(
            f"{scope} media limits are {MAX_IMAGES} images, {MAX_VIDEOS} videos and {MAX_AUDIOS} audio clips, "
            f"but received {counts['image']} images, {counts['video']} videos and {counts['audio']} audio clips. "
            "Reference only the media you need in the prompt, or keep fewer entries in the Media Loader."
        )
...
class MiniMaxH3Easy:
    ...
    def generate(cls, h3_bundle, mode, prompt, resolution, aspect_ratio, width, height, seconds, advanced, fps, keyframe_role, ref_image_size, reference_mention_mode, **kwargs):
        ...
        if mode == MODE_DIGITAL_HUMAN:
            visual_items, source_audio = _extract_digital_human_audio(items, "MiniMax H3 Aicg")
            visual_items, prompt = bind_reference_media(prompt, visual_items)
            _validate_reference_media(visual_items, "MiniMax H3 Aicg digital human mode")
            ...
            items, prompt = bind_reference_media(prompt, items)
            _sync_reference_video_cache_scope(items)
...
class MiniMaxH3EasyContextSegments:
    ...
    DESCRIPTION = (
        ...
        "output to MiniMax H3 Aicg Segment Sample."
    )
...
class MiniMaxH3EasyOutput:
    CATEGORY = "AICG3D/H3 工作流"
    FUNCTION = "unpack"
    RETURN_TYPES = ("CONDITIONING", "LATENT", "VAE", "VAE", "FLOAT", "AUDIO", "MODEL")
    RETURN_NAMES = ("positive", "latent", "video_vae", "audio_vae", "fps", "driving_audio", "model")
    DESCRIPTION = (
        "Unpack MiniMax H3 conditioning, AV latent, VAEs, FPS, the optional Digital Human "
        "driving audio, and the sampling model for the native sampler chain."
    )
...
class MiniMaxH3EasyRenderAdvanced:
    ...
    输入只需要上游 MiniMax H3 Aicg 的一条 h3_context（model / conditioning /
    latent / VAE / fps 都在 context 里），输出一条 VIDEO，直接接 SaveVideo。
    """
    ...
    RETURN_NAMES = ("video",)
    DESCRIPTION = (
        "一条龙出片：采样 + 视频解码 + 音频解码 + 合成，只输出一条 VIDEO。"
        "接 MiniMax H3 Aicg 的 h3_context，输出直接连 SaveVideo。"
    )
    ...
    @staticmethod
    def _decode_av(h3_context: MiniMaxH3Context, sampled: Any):
        """AV latent -> (frames, audio)：优先官方联合解码，失败回退标准解码。"""
        frames = None
        audio = None
        try:
        ...
        if frames is None or not isinstance(frames, torch.Tensor):
            frames = nodes.VAEDecode().decode(h3_context.video_vae, sampled)[0]
...
class MiniMaxH3EasySegmentRender:
    ...
    def render_chain(
        ...
        if not shots:
            raise ValueError(
                "Connect the H3 Context output of a MiniMax H3 Aicg Context Segments node"
            )
        bundle = plan.get("bundle")
        if not isinstance(bundle, MiniMaxH3Bundle):
...
class MiniMaxH3EasySegmentRefine:
    ...
    def refine_chain(
        ...
        refine_execution=None,
        target_width=0,
        target_height=0,
        latent_upscale_model=None,
        latent_upscale_scale=1.3,
        ...
        if not isinstance(segments, MiniMaxH3SegmentResult):
            raise ValueError("Connect the first-pass output from MiniMax H3 Aicg Segment Sample")
        model = _resolve_h3_model(h3_context, model)
        plan = segments.plan
...
class MiniMaxH3EasySegmentDecode:
    ...
    def decode_segments(cls, segments):
        if not isinstance(segments, MiniMaxH3SegmentResult):
            raise ValueError("Connect the segments output from MiniMax H3 Aicg Segment Sample")
        plan = segments.plan
...
class MiniMaxH3EasySelectedVideoRefine:
    ...
    ``MiniMax H3 Aicg`` context supplies the original prompt and any reference
...
class MiniMaxH3EasyAspectRatio:
    """Expose the Aicg node's resolved aspect ratio for downstream resolution controls."""

    CATEGORY = "AICG3D/H3 工作流"
    FUNCTION = "extract"
    RETURN_TYPES = ("*",)
    RETURN_NAMES = ("aspect_ratio",)
    DESCRIPTION = "Keep downstream resolution selectors aligned with MiniMax H3 Aicg without copying width or height."
    ...
    def rebuild(cls, h3_context, second_pass_video_latent):
        ...
        for source in h3_context.keyframe_sources:
            if not isinstance(source.image, torch.Tensor) or source.image.ndim != 4:
                raise ValueError("The original MiniMax H3 keyframe source is unavailable; run the MiniMax H3 Aicg node again")
...
NODE_DISPLAY_NAME_MAPPINGS = {
    ...
    "MiniMaxH3EasyMediaSplitter": "MiniMax H3 Aicg 媒体拆分",
    "MiniMaxH3Easy": "MiniMax H3 Aicg",
    "MiniMaxH3EasyContextSegments": "MiniMax H3 Aicg 上下文分段",
    "MiniMaxH3EasyOutput": "MiniMax H3 Aicg 输出",
    "MiniMaxH3EasySegmentRender": "MiniMax H3 Aicg 分段采样",
    "MiniMaxH3EasySegmentSampleSetup": "MiniMax H3 Aicg 采样设置",
    "MiniMaxH3EasySegmentStep": "MiniMax H3 Aicg 分段步骤",
    "MiniMaxH3EasySegmentCollect": "MiniMax H3 Aicg 分段汇总",
